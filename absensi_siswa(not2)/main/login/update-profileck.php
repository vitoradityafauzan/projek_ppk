...
// if name is less than 4 char then status is not ok
if (strlen($nama) < 4) {
$msg=$msg."<h1>Nama anda tidak boleh kurang dari 4 Huruf</h1><br>";
$status= "NOTOK";}	

if (filter_var($nama, FILTER_SANITIZE_STRING) === false) {
    echo("<h1>nama tidak boleh mengandung karakter selain huruf</h1><br>");
}

if (filter_var($email, FILTER_VALIDATE_EMAIL) === false){
    echo("<h1>Harap Masukan email yang benar!</h1><br>");
}

if($status<>"OK"){ // if validation failed
echo "<input type='button' value='Coba lagi' onClick='history.go(-1)'>";
